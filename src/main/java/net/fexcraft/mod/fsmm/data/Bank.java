...
public class Bank implements Manageable {
	...
	public boolean processAction(Action action, CommandSource log, Account sender, long amount, Account receiver, boolean included){
		Player player;
		long fee = 0, total;
		switch(action){
			case WITHDRAW:{
				if(sender == null){
					Config.chat(log, "Withdraw failed! Account is null.");
					Config.log(getName(log) + " -> player account is null.");
					return false;
				}
				if(amount <= 0){
					Config.chat(log, "Withdraw failed! Amount is null or negative. (T:" + amount + " || B:" + sender.getBalance() + ");");
					Config.log(getName(log) + " tried to withdraw a negative amount of money!");
					return false;
				}
				player = (Player)log;
				if(fees != null){
				...
				Config.chat(player, "Withdraw failed! Not enough money. (W:" + amount + " || B:" + sender.getBalance() + ");");
				Config.log(sender.getAsResourceLocation().toString() + " : Withdraw failed! Player does not have enough money. (T:" + amount + " || F:" + fee + ");");
				return false;
			}
			case DEPOSIT:{
				if(receiver == null){
					Config.chat(log, "Deposit failed! Account is null.");
					Config.log(getName(log) + " -> player account is null.");
					return false;
				}
				if(amount <= 0){
					Config.chat(log, "Deposit failed! Amount null or negative. (T:" + amount + " || I:" + ItemManager.countInInventory(log) + ");");
					Config.log(getName(log) + " tried to deposit a negative amount of money!");
					return false;
				}
				player = (Player)log;
				if(receiver.getBalance() + amount <= Long.MAX_VALUE){
					...
					if(ItemManager.countInInventory(player) - total >= 0){
						...
						Config.chat(player, "Deposit failed! Not enough money in Inventory. (D:" + amount + " || B:" + receiver.getBalance() + ");");
						Config.log(receiver.getAsResourceLocation().toString() + ": Deposit failed! Not enough money in Inventory. (D:" + amount + " || B:" + receiver.getBalance() + ");");
						return false;
					}
				}
				Config.chat(player, "Deposit failed! Result is above technical limit. (D:" + amount + " || B:" + receiver.getBalance() + ");");
				Config.log(receiver.getAsResourceLocation().toString() + " : Deposit failed! Result is above technical limit. (D:" + amount + " || B:" + receiver.getBalance() + ");");
				return false;
			}
			case TRANSFER:{
				if(sender == null){
					Config.chat(log, "Transfer failed! Sender is null.");
					Config.log(getName(log) + " -> sender account is null.");
					return false;
				}
				if(receiver == null){
					Config.chat(log, "Transfer failed! Receiver is null.");
					Config.log(getName(log) + " -> receiver account is null.");
					return false;
				}
				if(amount <= 0){
					Config.chat(log, "Transfer failed! Amount is null or negative. (T:" + amount + ");");
					Config.log(getName(log) + " tried to transfer a negative amount of money to " + receiver.getAsResourceLocation().toString() + "!");
					return false;
				}
				...
				Config.chat(log, "Transfer failed! Not enough money on sender Account.");
				Config.log(sender.getAsResourceLocation().toString() + " -> " + sender.getAsResourceLocation().toString() + " : Transfer failed! Sender doesn't have enough money. (T:" + amount + " || F:" + fee + ");");
				return false;
			}
			default:{
				Config.chat(log, "Invalid Bank Action. " + action.name() + " || " + getName(log) + " || "
	...
	private void log(Player player, Action action, long amount, long fee, long total, boolean included, Account sender, Account receiver){
		String s, r;
		switch(action){
			case DEPOSIT:
				s = sender.getTypeAndId();
				r = player.getGameProfile().getName();
				sender.addTransfer(new Transfer(amount, fee, included, action, sender.getName(), sender));
				break;
			case WITHDRAW:
				s = player.getGameProfile().getName();
	...
	public String getName(CommandSource sender){
		return sender == null ? "[NULL]" : ((Entity)sender).getName().toString();
	}
